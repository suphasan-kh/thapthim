...
class TestInputHardening < Minitest::Test
  ...
  def test_empty_and_nil
    ...
    assert_equal [], Thapthim.word_segment(nil)
    assert_equal [], Thapthim.syllable_segment("")
    assert_equal [], Thapthim.syllable_segment(nil)
    assert_equal [], Thapthim.tcc_segment("")
    assert_equal [], Thapthim.tcc_segment(nil)
  end

  def test_non_string_input_is_coerced_not_raised
    assert_equal ["12345"], Thapthim.word_segment(12345)
    refute_empty Thapthim.tcc_segment(12345)
  end

  def test_frozen_string_is_accepted
    assert_equal ["ฉัน", "รัก"], Thapthim.word_segment("ฉันรัก".freeze)
  end
  ...
  def test_embedded_nul_does_not_truncate
    text = "ฉัน" + NUL + "รัก"
    result = Thapthim.word_segment(text)
  ...
  def test_deterministic_for_identical_input
    ...
    assert_equal Thapthim.word_segment(s), Thapthim.word_segment(s)
  end

  # --- Thai-numeral grouping (years / phone / time stay whole) --------------------

  def test_thai_numerals_group_as_one_token
    assert_includes Thapthim.word_segment("ปี ๒๕๖๙"), "๒๕๖๙"
    assert_includes Thapthim.word_segment("เบอร์ ๐๘๑๒๓๔๕๖๗๘"), "๐๘๑๒๓๔๕๖๗๘"
    # interior separator (time / decimal) is kept whole, matching LST20's ASCII convention
    assert_includes Thapthim.word_segment("เวลา ๑๔.๓๐ น."), "๑๔.๓๐"
  end
  ...
  def test_known_oov_sentence
    result = Thapthim.word_segment("ฉันนั่งตากลม จากนั้นไปชมกอดอกไม้หลังบ้าน")
