...
# (Thapthim.segment / Thapthim.syllables). The input-hardening contract lives in
...
class TestSegment < Minitest::Test
  ...
  def test_basic_word_segmentation
    assert_equal ["ฉัน", "กิน", "ข้าว"], Thapthim.segment("ฉันกินข้าว")
  end

  def test_returns_array_of_strings
    result = Thapthim.segment("ฉันกินข้าว")
    assert_kind_of Array, result
    assert(result.all? { |t| t.is_a?(String) })
  end

  def test_syllables_basic
    refute_empty Thapthim.syllables("ฉันกินข้าว")
    assert(Thapthim.syllables("ฉันกินข้าว").all? { |t| t.is_a?(String) })
  end
  ...
  def test_syllables_is_lossless
    CORPUS.each do |s|
      assert_equal s, Thapthim.syllables(s).join,
  ...
  def test_tokens_are_non_empty
    CORPUS.each do |s|
      refute_includes Thapthim.segment(s), "", "no empty word token for #{s.inspect}"
      refute_includes Thapthim.syllables(s), "", "no empty syllable token for #{s.inspect}"
    end
  end
  ...
  def test_word_boundaries_are_subset_of_syllable_boundaries
    CORPUS.each do |s|
      word_bounds = cumulative_byte_offsets(Thapthim.segment(s))
      syl_bounds  = cumulative_byte_offsets(Thapthim.syllables(s))
  ...
  def test_every_token_is_valid_utf8_and_nul_free
    CORPUS.each do |s|
      (Thapthim.segment(s) + Thapthim.syllables(s)).each do |t|
  ...
  def test_segment_is_deterministic
    CORPUS.each do |s|
      assert_equal Thapthim.segment(s), Thapthim.segment(s), "segment unstable for #{s.inspect}"
      assert_equal Thapthim.syllables(s), Thapthim.syllables(s), "syllables unstable for #{s.inspect}"
    end
  end
  ...
  def test_whitespace_and_controls_preserved
    assert_equal "   ", Thapthim.segment("   ").join
    assert_equal "ก\nข\tค", Thapthim.segment("ก\nข\tค").join
  end
  ...
  def test_normalize_tokens_reassemble_into_normalized_text
    ...
    assert_equal normalized, Thapthim.segment(raw, normalize: true).join
    assert_equal normalized, Thapthim.syllables(raw, normalize: true).join
  end

  def test_normalize_does_not_mutate_input
    raw = "  ฉัน   กิน  ".dup
    before = raw.dup
    Thapthim.segment(raw, normalize: true)
    Thapthim.syllables(raw, normalize: true)
    assert_equal before, raw, "normalize: true must not mutate the caller's string"
  end

  def test_normalize_false_is_the_default
    raw = "  ฉัน   กิน  "
    assert_equal Thapthim.segment(raw), Thapthim.segment(raw, normalize: false)
    refute_equal Thapthim.segment(raw, normalize: true), Thapthim.segment(raw, normalize: false)
  end

  def test_frozen_input_with_normalize
    assert_equal ["ฉัน", "รัก"], Thapthim.segment("ฉันรัก".freeze, normalize: true)
  end

  # --- caller may freely mutate returned tokens (they are private copies) ----------

  def test_returned_tokens_are_independent_copies
    first = Thapthim.segment("ฉันกินข้าว")
    first.first << "XXX" rescue nil
    second = Thapthim.segment("ฉันกินข้าว")
